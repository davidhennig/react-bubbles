...
A token is a string of text that has been encrypted for the purpose of authentication. When a client makes a request for access to a server the server will provide the client with a token which will act like a key and give the client specifically instructed access based on said token.

- [ ] What steps can you take in your web apps to keep your data secure?

Authentication via tokens or protected routes is a simple and effective way to keep data secure on a web app. Think of the token as a key and the app as a door. It doesn’t inherently mean whatever is past that locked door is guaranteed to be 100% safe, but having that layer of protection in undoubtedly useful.

- [ ] Describe how web servers work.

This is a very large question, but I suppose the most succinct answer would be to say that a server houses data in a physical location on computers. Other computers can access the data housed on the server if it is on the same network and has appropriate access.

- [ ] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.

CRUD stands for Create Read Update Delete, and the methods of PUT POST GET DELETE can be mapped to it.
